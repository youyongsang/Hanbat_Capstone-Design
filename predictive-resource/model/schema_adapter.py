...
class SmartCSVLoader:
    ...
    def print_report(self):
        ...
        rps = r.get('rps_stats', {})
        print(
            f"  RPS 통계   : min={rps.get('min')}  max={rps.get('max')}"
    ...
    def _build_rps(self, df):
        ...
        if found is None:
            ...
            exclude = ['time', 'sec', 'min', 'hour', 'idx', 'index', 'id']
            candidates = [c for c in num_cols if not any(kw in c for kw in exclude)]
            if not candidates:
    ...
    def _build_day_of_week(self, df):
        ...
        if found is not None:
            ...
            df['day_of_week'] = pd.to_numeric(df['day_of_week'], errors='coerce').fillna(0).astype(int) % 7
        else:
